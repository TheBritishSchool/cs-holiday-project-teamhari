...
def register():
    if request.method == 'POST':
        ...
        if not email.endswith(ALLOWED_DOMAIN):
            flash("Registration is only allowed with TBS school emails.")
        ...
        try:
            cur.execute(
                "INSERT INTO users (username, password_hash, email, year) VALUES (%s, %s, %s, %s)",
                (username, hashed_password, email, year)
            )
