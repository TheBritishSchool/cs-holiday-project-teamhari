...
# --- EMAIL DISABLED MODE ---
def safe_send_email(*args, **kwargs):
    print("Email skipped (email system disabled).")
    return
...
# Initialize mail anyway so app doesn’t error
mail.init_app(app)
...
def home():
    ...
    for reply in all_replies:
        replies_dict.setdefault(reply[0], []).append(reply)
    ...
    for req in requests:
        ...
        requests_with_replies.append({
            ...
            "username": req[4],
            "image_path": req[5],
            "replies": replies_dict.get(req_id, [])
        })
    ...
    return render_template(
        ...
        requests=requests_with_replies,
        page=page,
...
def init_db():
    print(f"Connecting to MySQL at {app.config['MYSQL_HOST']}:{app.config['MYSQL_PORT']}")
    cur = mysql.connection.cursor()
...
def reply_request(request_id):
    ...
    cur = mysql.connection.cursor()
    cur.execute(
        "INSERT INTO Replies (user_id, request_id, message) VALUES (%s, %s, %s)",
        (current_user.id, request_id, message)
    )
    mysql.connection.commit()
    flash("Successfully posted reply!")
    ...
    # Email disabled; safe_send_email prevents errors
    safe_send_email()
